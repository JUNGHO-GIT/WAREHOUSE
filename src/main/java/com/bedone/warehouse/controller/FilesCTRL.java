...
public class FilesCTRL {
  ...
  public String saveFiles (
    ...
    utils.info ("filesParam : " + gson.toJson(filesParam));
  ...
  public void viewFiles (
    ...
    String path = gFileDir + fileUrl;
    String contentType = "";
    String fileExt = "";
    File file = new File(path);

    // 원래 파일이 없을 경우 기본 이미지로 대체 (resources/no-image)
    if (!file.exists() || fileUrl.equals("no-image.webp")) {
      ClassPathResource resource = new ClassPathResource("no-image.webp");
      file = resource.getFile();
    }

    // 확장자 형식이 맞지 않을 경우 기본 이미지로 대체 (resources/no-image)
