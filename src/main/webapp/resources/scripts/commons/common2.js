...
var fnAddDate = function (sDate, nDays) {
...
var fnShowSubMenu = function (cd) {
  ...
  var leftMenu = '';
  for (c = 0; c < G_subMenu[cd].length; c++) {
    if (c == 0) subMenu = '<div style="float:left;margin-right:10px;margin-top:2px;margin-left:' + G_subMenuMargin[cd] + 'px;">| </div>';
    subMenu += '<div onclick="fnExistDestPerms(\'' + G_subMenuLink[cd][c] + '\')" style="float:left;margin-right:5px;cursor:pointer;margin-top:2px;"> ' + G_subMenu[cd][c] + '</div><div onclick="fnMainOpen(\'' + G_subMenuLink[cd][c] + '\',\'' + G_subMenuLink[cd][c] + '\');" style="float:left;margin-right:10px;margin-top:2px;cursor:pointer;"> <img src="/img/newWin.png" style="vertical-align:middle"> </div><div style="float:left;margin-right:10px;margin-top:2px;"> | </div>';
  }
  $("#_subMenu").html(subMenu);
  $("#_subMenu").show(100);
  //alert(subMenu);
};
...
var fnSaveUserConfigTab = function () {
  ...
  param["pageNm"] = 'tabs';
  param["gridCd"] = '';
  param["configSeq"] = configSeq;
  param["config"] = tabs;
  param = JSON.stringify(param);
  //alert(param);

  var valUrl = "/act/saveUserConfig";
...
var fnFindCompany = function (findCompany, compCd, target) {
  if (event.keyCode != "13" && findCompany != "") {
    return; // 엔터일때만 수행
  }
  var param = "findCompany=" + findCompany;
...
var fnFindResrcNm = function (findResource, resrcCd, target) {

  if (event.keyCode != "13" && findResource != "") {
    return; // 엔터일때만 수행
  }

  var param = "findResource=" + findResource;

  var urlVal = "/act/findResource";
...
var fnFindProdNm = function (findProduct, prodCd, target) {
  ...
  var param = "findProduct=" + findProduct;
...
var fnFindCompanyCd = function (findNm, findCd, target) {
