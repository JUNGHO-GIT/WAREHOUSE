...
function fnSave() {
  ...
  for (let i = 0; i < colData.length; i++) {
    ...
    if (
      ...
      !validateField(row, "resrcType", "자재분류를 입력해 주세요") ||
...
function fnFilePicked(oEvent) {
  ...
  fileReader.onload = (e) => {
    ...
    wb.SheetNames.forEach((sheetName) => {
      ...
      const keyMap = [
        "resrcNm", "resrcType", "houseNm", "compNm", "maker", "unit", "quality", "option1", "option2", "protectedQty", "unitPrice", "remarks"
      ];
...
function fnExDownload() {
  const fileUrl = "resource_Xls_sample.xls";
...
jQuery(function($) {
  fnShowExcel();
  const oFileIn = document.getElementById("resourceXls");
