...
MyExcel.rgbToHex = function (r, g, b) {
  if (r == undefined || g == undefined || b == undefined) return undefined;
  return (componentToHex(r) + componentToHex(g) + componentToHex(b)).toUpperCase();
};

MyExcel.toExcelUTCTime = function (date1) {
...
MyExcel.toExcelLocalTime = function (date1) {
  var d2 = Math.floor(date1.getTime() / 1000);													// Number of seconds since JS epoch
  d2 = Math.floor(d2 / 86400) + 25569;																// Days since epoch plus difference in days between Excel EPOCH and JS epoch
  var seconds = date1.getUTCSeconds() + 60 * date1.getUTCMinutes() + 60 * 60 * date1.getUTCHours();		// Number of seconds of received hour
...
MyExcel.formats = BuiltInFormats;
...
for (var i = 0; i < MyExcel.borderStyles.length; i++) borderStylesUpper.push(MyExcel.borderStyles[i].toUpperCase());
...
function toBorderXml (b) {
  ...
  for (var i = 0; i < 4; i++) {
    ...
    if (vals[0] == "NONE") s = s + "/>";
      ...
      var border = MyExcel.borderStyles[borderStylesUpper.indexOf(vals[0])];
      if (border)
...
function escape (string) {
  if (typeof string != 'string') string = string == null ? '' : (string + '');
...
MyExcel.newExcel = function (defaultFont) {
  var excel = {};
