...
modify_application_properties() {
  ...
  # 만약 파일이 없다면 리턴
  if [ ! -f $file_path ]; then
    echo "application.properties file not found. Aborting upload."
    return
  fi
  ...
  # orders-url 설정 변경
  local ordersUrlPattern="^orders-url=http://localhost:8080/WAREHOUSE/orders"
  sed -i "s|$ordersUrlPattern|orders-url=http://104.196.212.101:8080/WAREHOUSE/orders|" "$file_path"
...
recover_application_properties() {
  ...
  local ordersUrlPattern="^orders-url=http://104.196.212.101:8080/WAREHOUSE/orders"
  sed -i "s|$ordersUrlPattern|orders-url=http://localhost:8080/WAREHOUSE/orders|" "$file_path"
